...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- !!! When modifying this stylesheet for a new spreadsheet, you should (in most cases) only need to  
            ...
            NB: * Each cell in the spreadsheet must contain data from only one source.
                ...
                * A bibl_ana column is also hard-coded into the stylesheet, but is not required. The values in this column determine what (if anything) goes into the person/@ana attribute,
...
-->
            ...
            <xsl:if test="$element-name!='none'">
                <xsl:element name="{$element-name}">
                    ...
                    <xsl:choose>
                        ...
                        <xsl:when test="matches(name(),'\.(Source|Edition|Translation|Version)_[0-9]*')">
                            ...
                            <xsl:variable name="source-name">
                                ...
                                <xsl:for-each select="$tokenized-column-name">
                                    <xsl:if test="matches(.,'^(Source|Edition|Translation|Version)_[0-9]*')">
                                        <xsl:value-of select="."/>
                                    </xsl:if>
                                </xsl:for-each>
                            </xsl:variable>
    ...
    <xsl:function name="syriaca:column-name">
        ...
        <xsl:analyze-string select="$column-name"
            regex="^([a-zA-Z0-9\-]+)(_([a-zA-Z0-9\-]+))?(\.((Source|Edition|Translation|Version)_[0-9]+))?(\.([a-zA-Z0-9\-]+))?$">
            <xsl:matching-substring>
                <elementName>
                        ...
                        <text>
                            <body>
                                ...
                                You can also change the order of these elements according to your preference, so long as it still produces valid TEI. -->
                                    ...
                                    <!--<xsl:copy-of select="$record-bibls/bibl[@type='lawd:Edition']"
                                        xpath-default-namespace="http://www.tei-c.org/ns/1.0"
                                        copy-namespaces="no"/>
                                    <xsl:copy-of select="$record-bibls/bibl[@type='lawd:Translation']"
                                        xpath-default-namespace="http://www.tei-c.org/ns/1.0"
                                        copy-namespaces="no"/>
                                    <xsl:copy-of select="$record-bibls/bibl[@type='lawd:Citation']"
                                        xpath-default-namespace="http://www.tei-c.org/ns/1.0"
                                        copy-namespaces="no"/>-->
    ...
    <xsl:template name="header" xmlns="http://www.tei-c.org/ns/1.0">
        ...
        <xsl:variable name="is-saint" select="contains(bibl_ana,'#syriaca-saint')"/>
        <xsl:variable name="is-author" select="contains(bibl_ana,'#syriaca-author')"/>
        <teiHeader>
            <fileDesc>
                <titleStmt>
    ...
    <xsl:template name="bibls" xmlns="http://www.tei-c.org/ns/1.0">
        ...
        <xsl:variable name="sources" select="distinct-values($column-mapping//@sourceUriColumn[.=$this-row[.!='']/name()])"/>
        <!-- creates a bibl for each of the source columns used in the spreadsheet. -->
        <xsl:variable name="sources-sorted">
            <xsl:for-each select="$sources">
                <xsl:sort select="index-of(('Edition','Translation','Version','Source'),replace(.,'_[0-9]+',''))"/>
                <xsl:copy-of select="."/>
            </xsl:for-each>
        </xsl:variable>
        ...
        <xsl:for-each select="$sources-sorted-tokenized">
            ...
            <xsl:for-each select="$source-column">
                ...
                <xsl:if test=".!=''">
                    ...
                    <!-- gets the lang from $column-mapping that names this column as its @sourceUriColumn -->
                    <xsl:variable name="langs"
                        ...
                        <xsl:variable name="bibl-url" select="concat($server-address,$bibl-or-ms,'/',.,'/tei')"/>
                        ...
                        <bibl>
                            ...
                            <xsl:choose>
                                ...
                                <xsl:when test="matches(name(),'Translation_')"><xsl:attribute name="type" select="'lawd:Translation'"/></xsl:when>
                                <xsl:when test="matches(name(),'Version_')"><xsl:attribute name="type" select="'lawd:Translation'"/></xsl:when>
                                <xsl:when test="matches(name(),'Source_')"><xsl:attribute name="type" select="'lawd:Citation'"/></xsl:when>
                                <xsl:when test="matches(name(),'Ms_')"><xsl:attribute name="type" select="'lawd:WrittenWork'"/></xsl:when>
                            </xsl:choose>  
                            ...
                            <ptr target="{concat('http://syriaca.org/',$bibl-or-ms,'/',.)}"/>
                            ...
                            <xsl:for-each select="$cited-ranges">
                                ...
                                <xsl:if test="matches($bibl-url,'#') and not(string-length($this-cited-range-content))">
                                    <xsl:variable name="msItem-id" select="replace($bibl-url,'^.*#','')"/>
                                    ...
                                    <xsl:if test="$locus">
                                        <xsl:element name="citedRange">
                                            ...
                                            <xsl:attribute name="from" select="$locus/locus/@from"/>
                                            <xsl:attribute name="to" select="$locus/locus/@to"/>
